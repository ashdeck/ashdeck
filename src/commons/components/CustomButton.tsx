...
const CustomButton = (props: CustomButtonProps) => {
    ...
    const variance = {
        ...
        outlined: "text-black border border-outline",
    ...
    const classes = twMerge(
        ...
        } py-2 px-4 font-[500] flex justify-center items-center gap-1 cursor-pointer rounded-lg font-outfit relative overflow-hidden capitalize transition duration-500 ${className}`,
    )
        ...
        <button disabled={(disabled || loading)} onClick={handleClick} className={classes} style={style} {...rest}>
            {!loading && (
                <div className="flex items-center justify-center">
                    {startIcon && <span className="mr-2 h-5 w-5 flex items-center justify-center">{startIcon}</span>}
                    {children}
                    {endIcon && (
                        <span className="ml-2 h-5 w-5 flex items-center justify-center">{endIcon && endIcon}</span>
                    )}
                </div>
            )}
